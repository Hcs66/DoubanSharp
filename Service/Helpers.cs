...
namespace DoubanSharp.Service
{
    ...
    {
        public static string ReplaceUriTemplateTokens(List<string> segments, string path)
        {
        ...
        public static string ReplacePathSegment(string path, string value, List<string> segments)
        {
        ...
        private static readonly Regex _escapeSegments = new Regex(@"\A(?:[?|&]\w*=)\Z", RegexOptions.Compiled);

        public static void EscapeDataContainingUrlSegments(IList<string> segments)
        {
    ...
    internal class StringHelpers
    {
        ...
        public static string Underscore(string camelCase)
        {
            var underscored = Regex.Replace(camelCase, UnderscoresPattern,
                                            new MatchEvaluator(m => string.Concat("_", m.Value.ToLowerInvariant())), RegexOptions.Compiled);
